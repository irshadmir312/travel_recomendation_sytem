...
# Load datasets and pre-trained model components
features = ['Name_x', 'State', 'Type', 'BestTimeToVisit', 'Preferences', 'Gender', 'NumberOfAdults', 'NumberOfChildren']
model = pickle.load(open('model.pkl', 'rb'))  # Correct path for model.pkl
label_encoders = pickle.load(open('label_encoders.pkl', 'rb'))  # Correct path for label_encoders.pkl

# Load destination data from CSV files
destinations_df = pd.read_csv("Expanded_Destinations.csv")  # Correct path for Expanded_Destinations.csv
userhistory_df = pd.read_csv("Final_Updated_Expanded_UserHistory.csv")  # Correct path for Final_Updated_Expanded_UserHistory.csv
df = pd.read_csv("final_df.csv")  # Correct path for final_df.csv
